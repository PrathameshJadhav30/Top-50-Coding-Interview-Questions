 public class Merge_Sort{
     //this function display the array
     public static void display(int [] arr, int size){
        for(int i =0; i < size; i++){
            System.out.print(arr[i] + " ");
        }
        System.out.println();
     }
     //main function of the program
    public static void main(String[] args) {
        int[] a ={ 12,8,7,4,14,38,98,64,15,72,67,84};
        int size = a.length;
        display(a , size);

        mergesort(a , 0 ,size-1);
        display(a , size);
    }
    //this function apply merging and sorting in the array
    static void mergesort(int[] a, int left, int right){
        int mid;
        if(left < right){
            //can also use mid = left + (right - left)/2
            //this can avoid data type overflow
            mid = (left + right)/2;

            //recursive calls to sort first half and secomd half array
            mergesort(a, left, mid);
            mergesort(a, mid + 1, right);
            merge(a, left , mid ,right);
        }
    }
    //after sorting this function merge the array
    static void merge(int[] arr, int left, int mid, int right){
        int i,j,k;
        int n1 = mid - left +1;
        int n2 = right - mid;

        //create temp arrays to store left and right sub-array
        int L[] = new int[n1];
        int R[] = new int[n2];

        //copying data to temp arrays L[] and R[]
        for(i = 0; i < n1; i++)
          L[i] = arr[left + i];
        for( j = 0; j < n2; j++)
          R[j] = arr[mid + 1 + j];

          //here we merge the temp arrays back into arr[i..r]
          i = 0; //strating index of L[i]
          j = 0; //starting index of R[i]
          k = left; //starting index of merged sub-array

          while(i < n1 && j < n2){
            //place the smaller item at arr[k] pos
            if(L[i] <= R[j]){
                arr[k] = L[i];
                i++;
            }
            else{
                arr[k] = R[j];
                j++;
            }
            k++;
          }
          //copy the remaining elements of l{}, if any
          while( i < n1){
            arr[k] = L[i];
            i++;
            k++;
          }
          //copy the elements of R[], if any
          while(j < n2){
            arr[k] = R[j];
            j++;
            k++;
          }
    }
}